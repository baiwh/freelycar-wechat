  ...
  <div class="pay-order">
      ...
      <div class="pay-order-card-item">
        ...
        <span class="pay-order-info-gray"
          >{{ orderId
            ...
            class="pay-order-info-blue copy"
    ...
    <div class="pay-order-card">
      <div class="pay-order-card-item">
        <span>车辆信息</span>
        <span class="pay-order-info-gray"
          >{{ consumerOrder.licensePlate }} {{ consumerOrder.carColor }} ·
          {{ consumerOrder.carBrand }}</span
        >
      </div>
      <div class="pay-order-card-item">
        <span>车主信息</span>
    ...
    <div
      class="pay-order-card"
      v-show="consumerOrder.state === 2 && consumerOrder.payState === 1"
    >
      <div class="pay-order-card-item">
        <span>支付方式</span>
        <!-- 修改为只有微信支付 -->
    ...
    <div class="pay-order-card">
      ...
      <div class="pay-order-card-item-second">
        <div v-for="(item, index) in consumerProjectInfos" :key="index">
          <span class="pay-order-info-gray">{{
            rspName + "-" + item.projectName
          }}</span>
          <!--<span class="pay-order-info-yellow">￥{{item.price}}</span>-->
        </div>
      </div>
      <div
        class="pay-order-card-item"
        v-show="consumerOrder.state === 2 || consumerOrder.state === 3"
      >
        <span class="pay-order-info-gray">
          <span class="pay-order-info-blue" @click="getImg('open')"
            >查看图片</span
          >
          <a :href="['tel:' + storeInfo.phone]">
            <img
              class="call-service"
              src="./../../assets/call-service.png"
              alt=""
            /><b class="pay-order-info-blue">联系服务商</b>
          </a>
        </span>
        <span
          >实付<b class="pay-order-info-yellow"
            >￥{{
              payWayInfo === "微信支付"
                ? consumerOrder.totalPrice
                : consumerOrder.memberPrice
            }}</b
          ></span
        >
      </div>
    ...
    <div
      ...
      v-show="consumerOrder.state === 2 && consumerOrder.payState === 1"
    >
      <span class="pay-order-info-yellow"
        >￥{{
          payWayInfo === "微信支付" || consumerOrder.firstPayMethod === 2
            ? consumerOrder.totalPrice
            : consumerOrder.memberPrice
        }}</span
      >
    ...
    <button
      class="blue-btn"
      v-show="
        consumerOrder.state === 0 ||
        consumerOrder.state === 1 ||
        consumerOrder.state === -1
      "
      @click="orderTracking"
    >
...
export default {
  ...
  methods: {
    ...
    changeImgIndex(index) {
      if (index == "-1") {
        this.staffOrderImgUrl = this.staffOrderImgUrls[
